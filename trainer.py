...
def main():
    ...
    for epoch in range(opt.nepoch):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            print('[%d: %d/%d] train loss: %f accuracy: %f' % (epoch, i, num_batch, loss.item(), correct.item()/float(opt.batchSize * 4096)))
            ...
            if i % 10 == 0:
                ...
                pred_choice = pred.data.max(1)[1]
                correct = pred_choice.eq(target.data).cpu().sum()
                print('[%d: %d/%d] %s loss: %f accuracy: %f' % (epoch, i, num_batch, blue('test'), loss.item(), correct.item()/float(opt.batchSize * 4096)))
        if correct.item()/float(opt.batchSize * 4096> best_acc):
